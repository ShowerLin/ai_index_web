...
export default function DemandToReturns(){
  return <section className="section conversion-section" id="conversion">
    ...
    <SectionSummary about="Connects model usage, token prices, infrastructure capacity and corporate returns in one analytical bridge." current="The retained OpenRouter snapshot shows substantial routed traffic; the refreshed token-price index has fallen 44% since June, while incremental ROIC remains below total ROIC for all five hyperscalers." conclusion="Demand exists, but the page cannot yet show how much revenue and profit each unit of usage produces. That missing conversion is the core uncertainty."/>
    <div className="conversion-call"><b>Investment assessment</b><p>The evidence indicates substantial routed usage, declining token prices and elevated capital deployment. The central issue is whether paid demand can generate sufficient incremental profit to support the expanding capital base.</p></div>
    ...
    <div className="conversion-detail">
      ...
      <aside className="conversion-gates"><span className="detail-kicker">THESIS MONITORING</span><div><b>Evidence of improving capital productivity</b><p>Cloud revenue and profit rise as capacity comes online, incremental ROIC improves, and cash coverage strengthens. An improvement driven primarily by lower spending would provide less support for the monetization thesis.</p></div><div><b>Evidence of persistent return pressure</b><p>Usage rises while segment margins, incremental ROIC and cash remaining after investment weaken. A sustained divergence would challenge the economic case for further expansion.</p></div><footer>Required evidence: cloud revenue, segment profit and realized model spending. A broad price index multiplied by token volume cannot establish revenue.</footer></aside>
    </div>
    <div className="conversion-boundary"><b>Scope and interpretation</b><p>OpenRouter is a retained high-frequency sample through 10 September rather than a measure of total market demand or company revenue; the refreshed workbook no longer contains its source sheet. The price series and macro inputs have different frequencies and may contain carried-forward observations. ROIC is consolidated rather than AI-specific. These measures are excluded from the Core and Satellite-adjusted atmosphere scores.</p></div>
  </section>;
}
