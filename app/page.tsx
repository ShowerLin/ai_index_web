...
export default function Home(){
  ...
  return <main id="top">
    <header className="topbar">
      ...
      <nav aria-label="Primary"><a href="#guide">Start here</a><a href="#chain">AI chain</a><a href="#pulse">Momentum</a><a href="#capex">Buildout</a><a href="#labs">Monetization</a><a href="#sustainability">Cash</a><a href="#roic">Returns</a><a href="#fundamentals">Market checks</a></nav>
      <div className="asof"><span className="live-dot"/>Data through · 10 Sep 2026</div>
    </header>

    <section className="hero">
      <div className="hero-copy">
        <div className="eyebrow">AI INVESTMENT ATMOSPHERE · INVESTMENT DASHBOARD</div>
    ...
    <section className="heatmap-overview" aria-labelledby="heatmap-title">
      ...
      <div className="heatmap-note"><b>Read-through:</b> Quarterly CapEx rose 25.3% in 2026Q2, with four of five companies spending more. That establishes the pace of investment; the cash coverage and ROIC sections test whether financial performance is keeping up.</div>
    </section>
    ...
    <section className="reader-guide" id="guide" aria-labelledby="guide-title">
      <div className="guide-intro">
        <div className="eyebrow">START HERE · THE ARGUMENT IN TWO MINUTES</div>
        ...
        <div className="guide-conclusion"><b>Current conclusion</b><span>Expansion is clear; funding remains available; evidence of payback is incomplete.</span></div>
      </div>
      <ol className="guide-path" aria-label="Suggested reading path">
        <li><a href="#pulse"><span>01</span><div><b>Is activity expanding?</b><small>Use the Pulse to judge momentum and breadth.</small></div></a></li>
      ...
      <p className="guide-orientation"><a href="#chain">New to the industry? Start with the AI value-chain map.</a> After the main argument, use <a href="#fundamentals">fundamentals and financing</a> to compare resilience across companies.</p>
    </section>
    ...
    <section className="section chain-section" id="chain">
      ...
      <div className="chain-note"><b>How to read the three signals:</b> Market is the equal-weight selected-period return (Strong ≥10%, Constructive ≥3%, Mixed &gt;−3%, otherwise Weak). Fundamental is the equal-weight composite from the <code>{industryChainSnapshotMetadata.fundamentalSheet}</code> company screen (Strong ≥65, Resilient ≥50, Mixed ≥35, otherwise Fragile). CDS pressure averages 1M and 3M spread changes for covered issuers only (Rising ≥+3 bp, Easing ≤−3 bp); coverage is always shown. One-day returns are unavailable because the current source reports zero for all 32 companies. Structural links show industry relationships rather than company-level supplier contracts or index weights.</div>
    </section>
    ...
    <section className="section fundamentals-section" id="fundamentals">
      ...
      <div className="fundamental-layout">
        ...
        <aside className="fundamental-method"><span className="detail-kicker">SCORING LOGIC</span><h3>Resilience first; spending momentum second.</h3><p>Resilience combines revenue growth (20%), EBITDA margin (25%), FCF / EBITDA (25%), inverse net debt / EBITDA (20%) and interest coverage (10%). Missing interest coverage is excluded and the remaining weights are renormalized.</p><div><b>Reporting period</b><p>Companies without an interim report remain on 2026-03-31. They receive no freshness penalty; the reporting cohort stays visible.</p></div><div><b>Use case</b><p>Use this screen to compare financial capacity across the supply chain. Strong spending growth alone does not establish an attractive return.</p></div></aside>
      </div>
    </section>
    ...
    <section className="section credit-section" id="credit">
      ...
      <div className="credit-layout">
        ...
        <aside className="credit-readthrough"><div><span className="detail-kicker">READ-THROUGH</span><h3>Recent tightening has only partly reversed the widening.</h3><p>The 12-issuer basket tightened by 1.4 bp over one month, while the three-month change remains 12.8 bp wider. Oracle and Dell drove much of the recent tightening; Intel, Oracle and Dell remain the largest three-month wideners.</p></div><div className="credit-method"><b>How to read it</b><p>The indicator gives equal weight to one-month and three-month spread changes. A positive score means credit conditions are improving. It is shown when at least eight issuers have comparable data.</p></div></aside>
      </div>
    </section>
    ...
    <section className="section financing-section" id="financing">
      ...
      <div className="financing-layout"><div className="chart-panel"><div className="chart-meta"><div><b>Monthly issuance activity</b><span>USD billions · bars show amount; hover for issue count</span></div></div><BondChart/><div className="chart-callout"><b>Financing backdrop:</b> the series describes market access and financing appetite. It is not issuer-specific debt attribution and is not combined into the 0–100 atmosphere score.</div></div><aside className="financing-note"><span className="detail-kicker">CURRENT READ-THROUGH</span><h3>One missing month prevents a clean YoY comparison.</h3><p>The latest continuous 12-month window is valid, but August 2025 is absent. Until that gap is restored, the website reports levels and month-on-month change without a rolling-12-month YoY rate.</p><div><b>What would strengthen it</b><p>Restoring the missing month would allow us to test whether issuance adds information beyond CDS and equity-price momentum.</p></div></aside></div>
    </section>

    <section className="section methodology" id="methodology">
      <div className="method-title"><div className="eyebrow">07 · METHODOLOGY & DATA NOTES</div><h2>How the signals are measured.</h2><p>The core index measures investment momentum. The experimental overlay adds financial quality and credit conditions. Cash coverage, lab revenue and ROIC provide separate evidence on sustainability and returns.</p></div>
      <SectionSummary about="Defines the core score, satellite adjustments, source definitions and missing-data rules." current="The official core remains 69.3; the experimental financial and credit overlay lowers the reading to 67.8." conclusion="Keeping momentum, financial resilience and returns separate prevents one strong signal from being mistaken for proof that the whole investment cycle is profitable."/>
      ...
      <div className="overlay-methodology">
        <div><span>EXPERIMENTAL OVERLAY FORMULA</span><h3>Adjusted = Core + Fundamental Δ + Credit Δ</h3><p><code>Fundamental Δ = 10% × (median Fundamental Composite − 50)</code></p><p><code>Credit score = clamp(50 + 2.5 × CDS Change Signal, 0, 100)</code></p><p><code>Credit Δ = 10% × (Credit score − 50)</code></p></div>
        <div className="overlay-method-grid"><article><span>Core</span><strong>{satelliteOverlay.core.toFixed(1)}</strong><small>Official five-pillar result</small></article><article><span>Fundamental Δ</span><strong>{satelliteOverlay.fundamentalDelta.toFixed(1)}</strong><small>Bounded to ±5 points by construction</small></article><article><span>Credit Δ</span><strong>{satelliteOverlay.creditDelta.toFixed(1)}</strong><small>Bounded to ±5 points by construction</small></article><article><span>Adjusted</span><strong>{formatIndex(satelliteOverlay.adjusted)}</strong><small>{satelliteOverlay.ready?"Clamped to 0–100":"Readiness gate failed"}</small></article></div>
        <footer><b>Missing-data rule:</b> no weight renormalization and no missing value is treated as an observed zero. If the financial or CDS readiness gate fails, the adjusted score is withheld. Bond issuance currently contributes 0.0 by an explicit methodology decision—not because issuance activity is assumed neutral.</footer>
      </div>
